...
source ("~/TFM/ATAC-seq/hysteresis_project/helper_functions.R")
...
dir <- paste0("~/TFM/ATAC-seq/hysteresis_project",names(comp_time))
...
for (i in 1:(length(dir))){
  ...
  ggsave(filename = paste0("../quality_control/", comp_time[[i]], "_no_filtered_RNA_QC.png"), plot = plot1, width = 15, height = 7)
  ...
  ggsave(filename = paste0("../quality_control/", comp_time[[i]], "_no_filtered_ATAC_QC.png"),
  ...
  saveRDS(seurat_obj, paste0("../r_objects/beforeQC/seurat_", comp_time[[i]], ".RDS"))
...
R_obj_dir <- "~/TFM/ATAC-seq/hysteresis_project/"                   #Doing it in local for now, when having access to cluster, do it like Tomàs
...
saveRDS(seurat_C22_0h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_C22_0h.RDS"))    #AFTER QC (ALL BELOW TOO)

seurat_C22_24h <- subset(
  x = seurat_C22_24h,
  subset = nCount_ATAC < 80000 &
    nCount_RNA < 25000 &
    nCount_ATAC > 500 &
...
saveRDS(seurat_C22_24h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_C22_24h.RDS"))

seurat_C22_48h <- subset(
  x = seurat_C22_48h,
  subset = nCount_ATAC < 80000 &
    nCount_RNA < 40000 &
    nCount_ATAC > 500 &
...
saveRDS(seurat_C22_48h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_C22_48h.RDS"))

seurat_C22_72h <- subset(
  x = seurat_C22_72h,
  subset = nCount_ATAC < 80000 &
    nCount_RNA < 20000 &
    nCount_ATAC > 500 &
...
saveRDS(seurat_C22_72h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_C22_72h.RDS"))
...
saveRDS(seurat_WT_0h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_WT_0h.RDS"))

seurat_WT_24h <- subset(
  x = seurat_WT_24h,
  subset = nCount_ATAC < 80000 &
    nCount_RNA < 30000 &
    nCount_ATAC > 500 &
...
saveRDS(seurat_WT_24h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_WT_24h.RDS"))

seurat_WT_48h <- subset(
  x = seurat_WT_48h,
  subset = nCount_ATAC < 80000 &
    nCount_RNA < 20000 &
    nCount_ATAC > 500 &
...
saveRDS(seurat_WT_48h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_WT_48h.RDS"))

seurat_WT_72h <- subset(
  x = seurat_WT_72h,
  subset = nCount_ATAC < 80000 &
    nCount_RNA < 20000 &
    nCount_ATAC > 500 &
...
saveRDS(seurat_WT_72h, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_WT_72h.RDS"))


###Merge the data by the time-points (two seurat, 1 for WT and 1 for C22)

# WT
seurat_merged_WT <- merge(
  x = seurat_WT_0h,
  y = list(seurat_WT_24h, seurat_WT_48h, seurat_WT_72h),
  add.cell.ids = c("WT_0h", "WT_24h", "WT_48h", "WT_72h"),
  merge.data = TRUE
)
DefaultAssay(seurat_merged_WT) <- "RNA"

saveRDS(seurat_merged_WT, paste("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_WT"))

seurat_merged_WT <- readRDS("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_WT")

# C22
seurat_merged_C22 <- merge(
  x = seurat_C22_0h,
  y = list(seurat_C22_24h, seurat_C22_48h, seurat_C22_72h),
  add.cell.ids = c("C22_0h", "C22_24h", "C22_48h", "C22_72h"),
  merge.data = TRUE
)
DefaultAssay(seurat_merged_C22) <- "RNA"

saveRDS(seurat_merged_C22, paste("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_C22"))

seurat_merged_C22 <- readRDS("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_C22")


###Add scores for the cells (for cell cycle, epithelial and mesenchymal phenotype, as well as stress, hybrid and EMT traits)

  # Cell cycle genes
s_mouse <- read.table("../genes_scores/s.genes.mouse", h = FALSE)
g2m_mouse <- read.table("../genes_scores/g2m.genes.mouse", h = FALSE)

  # Dissociation genes (stress markers)
dissoc_mouse <- read.table("../genes_scores/dissoc.genes.mouse", h = FALSE)

  # Epithelial marker genes
E_mouse <- read.table("../genes_scores/Toni_E_genes_mouse.txt", h = FALSE)

  # Mesenchymal marker genes
M_mouse <- read.table("../genes_scores/Toni_M_genes_mouse.txt", h = FALSE)

  # Hybrid marker genes
H_mouse <- read.table ("../genes_scores/Toni_Hybrid_genes_mouse.txt", h = FALSE)

  # EMT marker genes
EMT_mouse <- read.table ("../genes_socres/Toni_EMT_genes.txt", h = FALSE)
  ...
  #WT
seurat_merged_WT <- CellCycleScoring (seurat_merged_WT,
                                      s.features  = s_mouse$V1,
                                      g2m.features = g2m_mouse$V1,
                                      set.ident = TRUE)

seurat_merged_WT <- AddModuleScore (seurat_merged_WT,
...
seurat_merged_WT <- AddModuleScore (seurat_merged_WT, 
  ...
  #C22
seurat_merged_C22 <- CellCycleScoring (seurat_merged_C22,
                                       s.features = s_mouse$V1,
                                       g2m.features = g2m_mouse$V1,
                                       set.ident = TRUE)

seurat_merged_C22 <- AddModuleScore (seurat_merged_C22,
                                     features = list (dissoc_mouse$V1),
                                     name = "dissoc_score")
...
saveRDS(seurat_merged_WT, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_WT.RDS"))
saveRDS(seurat_merged_C22, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_C22.RDS"))
...
seurat_merged_WT <- NormalizeData (seurat_merged_WT, verbose = TRUE)
seurat_merged_WT <- FindVariableFeatures (seurat_merged_WT,
                                          selection.method = "vst",
                                          nfeatures = 3000,
                                          verbose = TRUE)

  # No regressing out any variable effect
seurat_merged_WT_no_reg_out <- ScaleData (seurat_merged_WT,
                                          verbose = FALSE) 

  # Regressing out cell cycle scores for visualization
seurat_merged_WT <- ScaleData (seurat_merged_WT,
                               verbose = FALSE,
                               vars.to.regress = c("S.Score", "G2M.Score")) 

  #PCA and UMAP
seurat_merged_WT_no_reg_out <- RunPCA (seurat_merged_WT_no_reg_out,
                                       npcs = 30, verbose = FALSE)

seurat_merged_WT <- RunPCA (seurat_merged_WT,
                             npcs = 30, verbose = FALSE)

seurat_merged_WT_no_reg_out <- RunUMAP (seurat_merged_WT_no_reg_out,
                                        reduction = "pca",
                                        dims = 1:30,
                                        verbose = TRUE,
...
seurat_merged_WT <- RunUMAP (seurat_merged_WT,
                            reduction = "pca",
                            dims = 1:30,
                            verbose = TRUE,
                            reduction.name = "umap_rna",
  ...
  # Normalization: log1p (counts/total_counts * scale_factor)
seurat_merged_C22 <- NormalizeData (seurat_merged_C22, verbose = TRUE)
seurat_merged_C22 <- FindVariableFeatures (seurat_merged_C22,
                                           selection.method = "vst",
                                           nfeatures = 3000,
                                           verbose = TRUE)

  # No regressing out any variable effect
seurat_merged_C22_no_reg_out <- ScaleData (seurat_merged_C22,
                                           verbose = TRUE)

  # Regressing out cell cycle scores for visualization
seurat_merged_C22 <- ScaleData (seurat_merged_C22,
                                verbose = FALSE,
                                vars.to.regress = c("S.Score", "G2M.Score"))

  # PCA and UMAP
seurat_merged_C22_no_reg_out <- RunPCA (seurat_merged_C22_no_reg_out,
                                        npcs = 30 , verbose = FALSE)

seurat_merged_C22 <- RunPCA (seurat_merged_C22,
                             npcs = 30, verbose = FALSE)

seurat_merged_C22_no_reg_out <- RunUMAP (seurat_merged_C22_no_reg_out,
                                         reduction = "pca",
                                         dims = 1:30,
                                         verbose = TRUE,
                                         reduction.name = "umap_rna",
...
seurat_merged_C22 <- RunUMAP (seurat_merged_C22,
                              reduction = "pca",
                              dims = 1:30,
                              verbose = TRUE,
                              reduction.name = "umap_rna",
...
##Plotting the umaps
  ...
  #WT
reg_out_umap <- DimPlot (seurat_merged_WT,
                         group.by = "old.ident",
                         cols = c("blue", "black", "yellow", "red")) + ggtitle("Cell cycle regressed out")

no_reg_out_umap <- DimPlot (seurat_merged_WT_no_reg_out,
                            group.by = "old.ident",
                            cols = c("blue", "black", "yellow", "red")) + ggtitle ("All variables effect")

reg_out_umap | no_reg_out_umap


  #C22
reg_out_umap <- DimPlot (seurat_merged_C22,
                         group.by = "old.ident",
                         cols = c("blue", "black", "yellow", "red")) + ggtitle("Cell cycle regressed out")

no_reg_out_umap <- DimPlot (seurat_merged_C22_no_reg_out,
                            group.by = "old.ident",
                            cols = c("blue", "black", "yellow", "red")) + ggtitle ("All variables effect")
...
## Normalize and visualize ATAC assay
  ...
  ###----WT----###
DefaultAssay (seurat_merged_WT) <- "ATAC"
DefaultAssay (seurat_merged_WT_no_reg_out) <- "ATAC"

# Standard workflow for pre-processing ATAC data
seurat_merged_WT <- FindTopFeatures (seurat_merged_WT, min.cutoff = 10) %>% #include features present in more than 10 cells
...
seurat_merged_WT_no_reg_out <- FindTopFeatures (seurat_merged_WT_no_reg_out,
                                                min.cutoff = 10) %>%
...
depthcor1 <- DepthCor(seurat_merged_WT)
depthcor2 <- DepthCor(seurat_merged_WT_no_reg_out)
depthcor1 | depthcor2

  
  ###----C22----###
DefaultAssay (seurat_merged_C22) <- "ATAC"
DefaultAssay (seurat_merged_C22_no_reg_out) <- "ATAC"

# Standard workflow for pre-processing ATAC data
seurat_merged_C22 <- FindTopFeatures (seurat_merged_C22,
                                      min.cutoff = 10) %>%
  # Normalization
  RunTFIDF() %>%
...
seurat_merged_C22_no_reg_out <- FindTopFeatures (seurat_merged_C22_no_reg_out,
                                                 min.cutoff = 10) %>%
...
depthcor1 <- DepthCor(seurat_merged_C22)
depthcor2 <- DepthCor(seurat_merged_C22_no_reg_out)
depthcor1 | depthcor2
...
saveRDS(seurat_merged_WT, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_WT.RDS"))
saveRDS(seurat_merged_WT_no_reg_out, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_WT_no_reg_out.RDS"))
saveRDS(seurat_merged_C22, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_C22.RDS"))
saveRDS(seurat_merged_C22_no_reg_out, paste0("~/TFM/ATAC-seq/hysteresis_project/seurat_merged_C22_no_reg_out.RDS"))
...
for (i in c(0:7,7)){
  try({
    marker_i <- FindConservedMarkers (seurat_all, ident.1 = i, grouping.var = "condition", only.pos = TRUE, verbose= TRUE)
    marker_i$cluster = i
    marker_all <- rbind (marker_all, marker_i)
    write.csv (marker_all, "conserved_markers_RNA_WNN_clusters_after_integration.csv")
    genes <- rownames (marker_all)
    gene.df <- bitr (genes, fromType = "SYMBOL", toType = c("ENSEMBL", "ENTREZID"), OrgDb = org.Mm.eg.db)
    enriched <- enrichGO (gene = gene.df$ENSEMBL, OrgDb = org.Mm.eg.db, keyType = "ENSEMBL", ont = "ALL", pAdjustMethod = "BH", qvalueCutoff = 0.1)
    filename <- paste0 ("conserved_markers_RNA_WNN_clusters_after_integration_enrichment_", i, ".csv")
    write.csv (enriched , filename)
  })
}
